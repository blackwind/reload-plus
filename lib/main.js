...
(function() {
  ...
  var bwApp = {
    ...
    onWidgetClick: function(event) {
      if (TO_ATTACH.includes(event.target.id) && !event.target.disabled) {
        if (event.button === 0 || event.button == 1) {
          ...
          var menu = Array.from(event.target.children).filter(c => c.getAttribute("anonid") == MENU_ANONID)[0];
    ...
    onWidgetContextMenu: function(event) {
      event.preventDefault();
      event.stopPropagation();
    },
    ...
    onWindowLoad: function(chromeWindow) {
      ...
      for (let id of TO_ATTACH) {
        ...
        if (reloadButton) {
          ...
          menu.setAttribute("anonid", MENU_ANONID);
          reloadButton.appendChild(menu);

          reloadButton.addEventListener("click", bwApp.onWidgetClick, true);
          reloadButton.addEventListener("contextmenu", bwApp.onWidgetContextMenu, true);
    ...
    onWindowUnload: function(chromeWindow) {
      ...
      for (let id of TO_ATTACH) {
        ...
        if (reloadButton) {
          ...
          for (let menu of reloadButton.getElementsByTagName("menupopup")) {
            if (menu.getAttribute("anonid") == MENU_ANONID)
              reloadButton.removeChild(menu);
          }

          reloadButton.removeEventListener("click", bwApp.onWidgetClick, true);
          reloadButton.removeEventListener("contextmenu", bwApp.onWidgetContextMenu, true);
  ...
  const MENU_ANONID = self.name + "-menu";
  const TO_ATTACH = [
    "ctr_reload-button",
