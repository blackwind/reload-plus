...
(function() {
  ...
  var bwApp = {
    executeAction: function(action) {
      if (action.isBetween(1, MAX_ACTION)) {
        ...
        var targetWindows = (bwPreferences.get("targetAllWindows") ? Array.prototype.map.call(windows, w => _(w)) : [chromeWindow]);
        ...
        switch (action) {
          ...
          case 8:
            // Reload All Tabs
            for (var targetWindow of targetWindows) {
              for (var tab of targetWindow.gBrowser.tabs) {
    ...
    hotkeys: {
      bind: function() {
        ...
        for (var meta of vals) { for (var shift of vals) { for (var alt of vals) { for (var ctrl of vals) {
    ...
    retryInterval: function(tab, delay) {
      ...
      var retryInterval = {
        ...
        start: function() {
          ...
          browserMM.addMessageListener("response", function callee(response) { browserMM.removeMessageListener("response", callee);
  ...
  for (var w of windows) bwApp.onWindowLoad(_(w));
