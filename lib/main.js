(function() {
  ...
  var bwApp = {
    executeAction: function(action) {
      if (bwData.isBetween(action, 1, MAX_ACTION)) {
        ...
        switch (action) {
          ...
          case 11:
            ...
            for each (var tab in chromeWindow.gBrowser.tabs) {
              if (!tab.hidden && /^about:neterror\?/.test(tab.linkedBrowser._contentWindow.document.documentURI))
                tab.linkedBrowser.reload();
            }
            break;
          case 12:
            ...
            if (tab._retryInterval) {
              ...
              try {
                ...
                if (bwData.isBetween(delay, 1, MAX_DURATION)) {
      ...
      for (var i in actions) {
        ...
        if (callback && i == 0) {  // jshint ignore:line
          ...
          if (typeof(callback) == "function") {
            ...
            item.setAttribute("style", "list-style-image: url('" + require("sdk/self").data.url("action" + i + ".png") + "');");
    ...
    delegate: {
      ...
      onTrack: function(chromeWindow) {
        ...
        for each (var button in ATTACH_LIST) {
      ...
      onUntrack: function(chromeWindow) {
        ...
        for each (var button in ATTACH_LIST) {
          ...
          if (reloadButton) {
            ...
            if (menu) reloadButton.removeChild(menu);
    ...
    onOptionsDisplayed: function() {
      var document = winUtils.activeWindow._content.document;
    ...
    onPageLoad: function(event) {
      ...
      if (chromeWindow.location == "chrome://browser/content/browser.xul" && !document.defaultView.frameElement) {
        ...
        var ignoredErrors = ["fileNotFound", "malformedURI", "netOffline", "notCached", "protocolNotFound", "redirectLoop", "remoteXUL"];

        if (errorCode && !bwData.inArray(errorCode, ignoredErrors)) {
          var delay = bwPreferences.get("autoRetryDelay");
          bwApp.retryInterval(chromeWindow, tab, delay, errorCode);
    ...
    retryInterval: function(chromeWindow, tab, delay, isErrorPage) {
      if (tab.tagName != "tab" || !bwData.isBetween(delay, 1, MAX_DURATION))
        return false;
  ...
  var bwData = {
    ...
    createArray: function() {
      ...
      for each (var arg in arguments) {
        result = result.concat(Array.prototype.slice.call(arg));
      }
      return result;
    },
  ...
  var bwDOM = {
    getAllDocuments: function(document) {
      var result = [document];
      var frames = bwData.createArray(document.getElementsByTagName("frame"), document.getElementsByTagName("iframe"));
  ...
  const ATTACH_LIST = ["ctr_reload-button", "ctraddon_reload-button", "reload-button", "urlbar-reload-button"];
  ...
  bwPreferences.init({
    ...
    "autoRetryDelay":    {"default": 0, "min": 0, "max": MAX_DURATION},
  ...
  winUtils.WindowTracker(bwApp.delegate);
})();
