(function() {
  ...
  var bwApp = {
            ...
            if (tab._retryInterval) {
              ...
              try {
                ...
                if (bwData.isBetween(delay, 1, 86400)) {
                  ...
                  tab._retryInterval = new bwApp.retryInterval(tab, delay).obj;
                }
              } catch (e) { }
            }
            break;
    ...
    onOptionsDisplayed: function(document) {
      ...
      /* No simple-prefs workaround: */ document.getElementById("reloadplus-functionKey").addEventListener("command", function() bwApp.hotkeys.bind());
    },
    ...
    onPageLoad: function(chromeWindow, document) {
      ...
      if (errorCode && !bwData.inArray(errorCode, ignoredErrors)) {
        ...
        /*var result =*/ new bwApp.retryInterval(tab, delay, errorCode).obj;
    ...
    retryInterval: function(tab, delay, isErrorPage) {
        ...
        start: function() {
          ...
          timer = window.setInterval(function() {
            if (--delayLeft <= 0) {
              ...
              if (isErrorPage) {
                window.document.getElementById("errorTryAgain").click();
              } else {
  ...
  var bwData = {
    ...
    parseTime: function(time) {
      if (time < 60) {
        ...
        var h = parseInt(time / 3600); h = (h > 0 ? h+":" : "");
        var m = parseInt(time % 3600 / 60); m = (h && m < 10 ? "0"+m+":" : m+":");
        var s = parseInt(time % 3600 % 60); s = (s < 10 ? "0"+s : s);
        return h + m + s;
      }
    },
  }
