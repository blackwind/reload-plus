(function() {
  ...
  var bwApp = {
    ...
    createMenu: function(document, callback) {
      var document = (document ? document : _(windows.activeWindow).document);
    ...
    onHttpResponse: function(event) {
      try {
        ...
        if (!TO_IGNORE.statusCodes.includes(httpChannel.responseStatus) && !TO_IGNORE.statusTexts.includes(httpChannel.responseStatusText)) {
          ...
          var currentURI = _(tab).url;
          if (currentURI == httpChannel.name || (["about:blank", "about:newtab"].includes(currentURI) && typeof(tab._unsuccessful) == "undefined"))
    ...
    onOptionsDisplayed: function() {
      var document = _(windows.activeWindow).content.document;
    ...
    retryInterval: function(tab, delay) {
      ...
      var retryInterval = {
        ...
        start: function() {
          // TODO: Remove this e10s workaround when bug 967873 is fixed.
          retryInterval.stop();

          browserMM.sendAsyncMessage("createLoader");
          browserMM.addMessageListener("response", function(response) { browserMM.removeMessageListener("response", arguments.callee);
  ...
  // TODO: Remove polyfill when other changes break compatibility with Fx26-39.
  ...
  for each (var w in windows) bwApp.onWindowLoad(_(w));
  windows.on("open", function(w) bwApp.onWindowLoad(_(w)));
  windows.on("close", function(w) bwApp.onWindowUnload(_(w)));
})();
